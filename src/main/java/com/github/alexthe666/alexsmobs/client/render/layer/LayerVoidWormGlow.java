    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T worm, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (isGlowing(worm) || special) {
            if(special){
                ...
                VertexConsumer consumer = bufferIn.getBuffer(RenderType.entityTranslucentEmissive(texture));
                this.layerModel.renderToBuffer(matrixStackIn, consumer, 240, OverlayTexture.NO_OVERLAY);
            }else{
                float f = getAlpha(worm);
                this.getParentModel().renderToBuffer(matrixStackIn, bufferIn.getBuffer(RenderType.eyes(texture)), 240, LivingEntityRenderer.getOverlayCoords(worm, 1.0F));
            }
        }
    }
