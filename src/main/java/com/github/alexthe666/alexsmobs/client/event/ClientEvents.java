...
public class ClientEvents {
    ...
    public void onOutlineEntityColor(EventGetOutlineColor event) {
        ...
        if (event.getEntityIn() instanceof ItemEntity && ((ItemEntity) event.getEntityIn()).getItem().is(AMTagRegistry.VOID_WORM_DROPS)){
            ...
            float f = (float) (Math.cos(0.4F * (event.getEntityIn().tickCount + Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true))) + 1.0F) * 0.5F;
    ...
    public void onPreRenderEntity(RenderLivingEvent.Pre event) {
        ...
        if (event.getEntity() instanceof WanderingTrader && event.getEntity().getType() == EntityType.WANDERING_TRADER) {
            if (event.getEntity().getVehicle() instanceof EntityElephant) {
                // TODO 1.21: model field is now protected, needs getModel() or mixin
        ...
        if (event.getEntity().hasEffect(AMEffectRegistry.CLINGING) && event.getEntity().getEyeHeight() < event.getEntity().getBbHeight() * 0.45F || event.getEntity().hasEffect(AMEffectRegistry.DEBILITATING_STING) && event.getEntity().getTags().contains("arthropod") && event.getEntity().getBbWidth() > event.getEntity().getBbHeight()) {
            event.getPoseStack().pushPose();
    ...
    public void onPostGameOverlay(RenderGuiLayerEvent.Post event) {
        if(renderStaticScreenFor > 0){
            ...
            if (event.getName().equals(VanillaGuiLayers.CAMERA_OVERLAYS)) {
                ...
                float ageInTicks = Minecraft.getInstance().level.getGameTime() + Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true);
