...
public class RenderFarseer extends MobRenderer<EntityFarseer, ModelFarseer> {
    ...
    public void render(EntityFarseer entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        // RenderNameTagEvent removed in 1.21
        ...
        if(entityIn.getAnimation() == EntityFarseer.ANIMATION_EMERGE){
            ...
            VertexConsumer portalConsumer = bufferIn.getBuffer(RenderType.entityTranslucentEmissive(PORTAL_TEXTURES[portalTexture]));
    ...
    private void renderFarseerModel(PoseStack matrixStackIn, MultiBufferSource source, RenderType defRenderType, float partialTicks, int packedLightIn, int overlayColors, float alphaIn, EntityFarseer entityIn) {
        if(entityIn.hasLaser()){
            // In 1.21, VertexMultiConsumer doesn't work with entity rendering - use single render type
            VertexConsumer eyeConsumer = source.getBuffer(RenderType.entityTranslucentEmissive(TEXTURE_EYE));
            EYE_MODEL.renderToBuffer(matrixStackIn, eyeConsumer, packedLightIn, NO_OVERLAY);
        }
        ...
        if(hurt > 0){
            ...
            // In 1.21, VertexMultiConsumer doesn't work with entity rendering - use single render type
            VertexConsumer scarsConsumer = source.getBuffer(RenderType.entityTranslucentEmissive(TEXTURE_SCARS));
    ...
    protected void setupRotations(EntityFarseer farseer, PoseStack matrixStackIn, float f1, float f2, float f3) {
        float invCameraAmount = 1F - farseer.getFacingCameraAmount(Minecraft.getInstance().getTimer().getGameTimeDeltaPartialTick(true));
