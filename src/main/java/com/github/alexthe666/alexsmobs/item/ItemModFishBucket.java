...
public class ItemModFishBucket extends MobBucketItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        EntityType fishType = fishTypeSupplier.get();
        if (fishType == AMEntityRegistry.LOBSTER.get()) {
            CompoundTag compoundnbt = /* TODO 1.21: getTag() removed */ null;
    ...
    private void spawnFish(ServerLevel serverLevel, ItemStack stack, BlockPos pos) {
        Entity entity = fishTypeSupplier.get().spawn(serverLevel, stack, (Player)null, pos, MobSpawnType.BUCKET, true, false);
        if (entity instanceof Bucketable) {
            Bucketable bucketable = (Bucketable)entity;
            bucketable.loadFromBucketTag(stack.getOrDefault(net.minecraft.core.component.DataComponents.BUCKET_ENTITY_DATA, net.minecraft.world.item.component.CustomData.EMPTY).copyTag());
