...
public class AMBlockItem extends BlockItem implements CustomTabBehavior {
    ...
    public void onDestroyed(ItemEntity p_150700_) {
        if (this.blockSupplier.get() instanceof ShulkerBoxBlock) {
            ...
            // TODO 1.21: getBlockEntityData removed - use DataComponents
            CompoundTag compoundtag = null; // getBlockEntityData(itemstack);
            if (compoundtag != null && compoundtag.contains("Items", 9)) {
                ListTag listtag = compoundtag.getList("Items", 10);
                // TODO 1.21: ItemUtils.onContainerDestroyed API changed
                // ItemUtils.onContainerDestroyed(p_150700_, listtag.stream().map(CompoundTag.class::cast).map((tag) -> ItemStack.parseOptional(net.minecraft.core.registries.BuiltInRegistries.ITEM.asLookup(), tag)));
            }
        }
    }
    ...
    public boolean canBeHurtBy(DamageSource damage) {
        return true /* TODO 1.21: canBeHurtBy signature changed - super.canBeHurtBy(stack, damage) */ && (this != AMBlockRegistry.TRANSMUTATION_TABLE.get().asItem() || !damage.is(DamageTypeTags.IS_EXPLOSION));
    }
