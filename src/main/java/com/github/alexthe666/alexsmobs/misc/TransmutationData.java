...
public class TransmutationData {
    ...
    public CompoundTag saveAsNBT(net.minecraft.core.HolderLookup.Provider provider){
        ...
        for(Object2DoubleMap.Entry<ItemStack> entry : itemstackData.object2DoubleEntrySet()) {
            ...
            tag.put("Item", entry.getKey().save(provider, new CompoundTag()));
    ...
    public static TransmutationData fromNBT(CompoundTag compound){
        ...
        if (compound.contains("TransmutationData")) {
            ...
            for (int i = 0; i < listtag.size(); ++i) {
                ...
                try{
                    ItemStack from = ItemStack.parseOptional(net.minecraft.core.HolderLookup.Provider.create(java.util.stream.Stream.empty()), innerTag.getCompound("Item"));
                    if(!from.isEmpty()){
