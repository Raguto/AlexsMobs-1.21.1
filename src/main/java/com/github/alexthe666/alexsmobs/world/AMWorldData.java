...
public class AMWorldData extends SavedData {
    ...
    public static AMWorldData get(Level world) {
        if (world instanceof ServerLevel) {
            ...
            if(fromMap == null){
                ...
                AMWorldData data = storage.computeIfAbsent(new SavedData.Factory<>(AMWorldData::new, (tag, provider) -> AMWorldData.load(tag)), IDENTIFIER);
                if (data != null) {
    ...
    public int getWaterHeight(NoiseBasedChunkGenerator generator, RandomState rand, int x, int z, LevelHeightAccessor level) {
        // TODO 1.21: settings field is private
        NoiseSettings noisesettings = null; // generator.settings.value().noiseSettings();
        int i = Math.max(noisesettings.minY(), level.getMinBuildHeight());
        int j = Math.min(noisesettings != null ? noisesettings.minY() + noisesettings.height() : 256, level.getMaxBuildHeight());
        int k = Mth.floorDiv(i, noisesettings.getCellHeight());
        int l = Mth.floorDiv(j - i, noisesettings.getCellHeight());
        return level.getMinBuildHeight(); // TODO 1.21: iterateNoiseColumn is protected
    }
}
